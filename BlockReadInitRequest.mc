...
module Pinion
{
    class BlockReadInitRequest extends Request
    {
        ...
        public function decodeResponse(bytes as Lang.ByteArray) as Request.ResponseResult
        {
            ...
            {
                System.println("Pinion: BlockReadInitRequest response error " + bytesToHex(bytes));
                return RESPONSE_FAILURE;
            }

            if(bytes[0] != PINION_BLOCK_REPLY)
            {
                System.println("Pinion: BlockReadInitRequest response is not a reply");
                return RESPONSE_FAILURE;
            }

            if(bytes[1] != 0)
            {
                System.println("Pinion: BlockReadInitRequest response is not an init");
                return RESPONSE_FAILURE;
            }
            ...
            {
                System.println("Pinion: BlockReadInitRequest response is for the wrong address");
                return RESPONSE_FAILURE;
            }
